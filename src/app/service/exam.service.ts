...
export class ExamService {
  ...
  addExpr(examId: number | string, exprId: number | string, vmIdPrefix: number | string) {
    return this.httpClient.post<number>('api/exam/addExpr', { examId, exprId, vmIdPrefix });
  }

  getExprs(examId: number | string) {
    return this.httpClient
      .get<Expr[]>('api/exam/exprs', { examId })
      .pipe(
        map((resp) => {
          return resp.map((e) => {
